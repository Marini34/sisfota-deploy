<x-app-layout>
    @section('title', 'Riwayat Penilaian Seminar Proposal')
    @section('head')
                ...
                <table id="myTable" class="table table-striped table-hover table-bordered">
                    ...
                    <tbody>
                        @foreach ($seminarSidangs as $sempro)
                            ...
                            <dialog id="confirmDialog_{{ $sempro->slug }}"
                                ...
                                <div class="relative">
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <form action="{{ route('dosen.sempro.penilaian.batal', $sempro->slug) }}"
                            ...
                            <tr>
                                <td class="min-w-32">{{ $sempro->mahasiswa->nama_lengkap }}</td>
                                <td class="">{{ $sempro->mahasiswa->nim }}</td>
                                <td class="min-w-40">{{ $sempro->tugas_akhir->judul }}</td>
                                <td class="">
                                    {{ \Carbon\Carbon::parse($sempro->tanggal_pelaksanaan)->translatedFormat('l, d F Y') }}
                                </td>
                                ...
                                <td class="whitespace-nowrap text-center">
                                    <a href="/dosen/penilaian-sempro/detail/{{ $sempro->slug }}"
                                    ...
                                    @if (in_array($sempro->tugas_akhir_id, $semprosIds))
                                        <a href="/dosen/penilaian-sempro/edit/{{ $sempro->slug }}"
                                            data-tooltip-target="nilai_{{ $sempro->slug }}"
                                            data-tooltip-placement="bottom" type="button"
                                        ...
                                        <div id="nilai_{{ $sempro->slug }}" role="tooltip"
                                            class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                            Riwayat penilaian
                                            <div class="tooltip-arrow" data-popper-arrow></div>
                                        </div>

                                        <button data-tooltip-target="batal_{{ $sempro->slug }}"
                                            data-tooltip-placement="bottom" type="button"
                                            onclick="showConfirm('{{ $sempro->slug }}')" disabled
                                        ...
                                        <div id="batal_{{ $sempro->slug }}" role="tooltip"
                                            class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                            Tidak bisa membatalkan, status semhas telah diterima
                                            <div class="tooltip-arrow" data-popper-arrow></div>
                                        </div>
                                    @else
                                        <a href="/dosen/penilaian-sempro/edit/{{ $sempro->slug }}"
        ...
        <script type="text/javascript">
            let table = new DataTable('#myTable', {
                ...
                language: {
                    ...
                    emptyTable: "Belum ada seminar proposal yang dinilai"
                },
                columnDefs: [{
                        targets: [-1],
                        width: "5%",
