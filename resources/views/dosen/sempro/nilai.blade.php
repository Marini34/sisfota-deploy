<x-app-layout>
    ...
    <x-main-card>
        @if ($parameters->count() && $totalPersentase == 100)
            <h1 class="text-lg font-bold mb-5">Penilaian Seminar Proposal {{ $sempro->mahasiswa->nama_lengkap }}</h1>
            <form action="{{ route('dosen.sempro.penilaian.store', $sempro->slug) }}" method="post">
                @csrf
