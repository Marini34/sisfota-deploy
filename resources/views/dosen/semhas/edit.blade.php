<x-app-layout>
    @section('title', 'Edit Penilaian Seminar Hasil')
    @section('head')
    ...
    <x-main-card>
        @if ($parameters->count() && $totalPersentase == 100)
            <h1 class="text-lg font-bold mb-5">@if ($sidang) Riwayat @else Edit @endif Penilaian Seminar Hasil {{ $semhas->mahasiswa->nama_lengkap }}</h1>
            <form action="{{ route('dosen.semhas.penilaian.edit.store', $semhas->slug) }}" method="post">
                @csrf
                ...
                @if ($sidang)
                <a href="/dosen/penilaian-semhas/riwayat" type="button"
