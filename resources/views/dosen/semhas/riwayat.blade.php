<x-app-layout>
    @section('title', 'Riwayat Penilaian Seminar Hasil')
    @section('head')
        ...
        <div class="text-gray-900 dark:text-gray-100">
            <div class="flex items-center justify-between mb-4">
                <h5 class="mb-2 text-2xl font-medium tracking-tight text-gray-900 dark:text-white">
                    Riwayat Penilaian Seminar Hasil
                </h5>
            </div>
            <div class="table-responsive text-base">
                <table id="myTable" class="table table-striped table-hover table-bordered">
                    ...
                    <tbody>
                        @foreach ($seminarSidangs as $semhas)
                                ...
                                <div class="relative">
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                            Apakah anda yakin untuk membatalkan penilaian ini?
                                        </h3>
                                        <form action="{{ route('dosen.semhas.penilaian.batal', $semhas->slug) }}"
                            ...
                            <tr>
                                <td class="min-w-32">{{ $semhas->mahasiswa->nama_lengkap }}</td>
                                <td class="">{{ $semhas->mahasiswa->nim }}</td>
                                <td class="min-w-40">{{ $semhas->tugas_akhir->judul }}</td>
                                <td class="">
                                    {{ \Carbon\Carbon::parse($semhas->tanggal_pelaksanaan)->translatedFormat('l, d F Y') }}
                                </td>
                                ...
                                <td class="whitespace-nowrap text-center">
                                    <a href="/dosen/penilaian-semhas/detail/{{ $semhas->slug }}"
                                    ...
                                    @if (in_array($semhas->tugas_akhir_id, $semhasIds))
                                        <a href="/dosen/penilaian-semhas/edit/{{ $semhas->slug }}"
                                            data-tooltip-target="nilai_{{ $semhas->slug }}"
                                            data-tooltip-placement="bottom" type="button"
                                            class="cursor-pointer text-white bg-gradient-to-r from-blue-400 via-blue-500 to-blue-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-8 h-7 inline-flex items-center justify-center">
                                            <i class="bi bi-eye"></i>
                                        </a>

                                        <div id="nilai_{{ $semhas->slug }}" role="tooltip"
                                            class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                            Riwayat Penilaian
                                            <div class="tooltip-arrow" data-popper-arrow></div>
                                        </div>

                                        <button data-tooltip-target="batal_{{ $semhas->slug }}"
                                            data-tooltip-placement="bottom" type="button"
                                            onclick="showConfirm('{{ $semhas->slug }}')" disabled
                                            class="cursor-not-allowed text-white bg-gradient-to-r from-slate-400 via-slate-500 to-slate-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-slate-300 dark:focus:ring-slate-800 font-medium rounded-lg text-sm w-8 h-7 inline-flex items-center justify-center">
                                            <i class="bi bi-arrow-counterclockwise"></i>
                                        </button>

                                        <div id="batal_{{ $semhas->slug }}" role="tooltip"
                                            class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                            Tidak bisa membatalkan, status sidang telah diterima
                                            <div class="tooltip-arrow" data-popper-arrow></div>
                                        </div>
                                    @else
                                        <a href="/dosen/penilaian-semhas/edit/{{ $semhas->slug }}"
                                            data-tooltip-target="nilai_{{ $semhas->slug }}"
                                            data-tooltip-placement="bottom" type="button"
                                            class="cursor-pointer text-white bg-gradient-to-r from-blue-400 via-blue-500 to-blue-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm w-8 h-7 inline-flex items-center justify-center">
        ...
        <script type="text/javascript">
            let table = new DataTable('#myTable', {
                ...
                language: {
                    ...
                    emptyTable: "Belum ada seminar hasil yang dinilai"
                },
                columnDefs: [{
                        targets: [-1],
                        width: "5%",
