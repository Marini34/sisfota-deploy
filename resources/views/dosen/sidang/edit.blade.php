<x-app-layout>
    @section('title', 'Edit Penilaian Sidang Akhir')
    @section('head')
    ...
    @endsection
    @if (
        $skripsiParameters->count() &&
            $totalPersentaseSkripsi == 100 &&
            ($artikelParameters->count() && $totalPersentaseArtikel == 100) &&
            ($presentasiParameters->count() && $totalPersentasePresentasi == 100))
        <x-main-card>
            <h1 class="text-2xl font-bold mb-5">Edit Penilaian Sidang Akhir {{ $sidang->mahasiswa->nama_lengkap }}</h1>
            <br>
            <form action="{{ route('dosen.sidang.penilaian.edit.store', $sidang->slug) }}" method="post">
                @csrf
                    ...
                    <table id="myTable" class="table table-striped table-hover table-bordered">
                        <thead class="text-base uppercase text-slate-800">
                            <tr class="bg-slate-300 rounded-2xl text-center">
                                <th style="width: 11rem;">Parameter</th>
                                <th>Deskripsi</th>
                                <th>Persentase</th>
                                <th>Nilai</th>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <input required type="number" min="0" max="100"
                                            name="nilaiSkripsi[{{ $index }}]"
                                            id="nilaiSkripsi[{{ $index }}]"
                                            value="{{ isset($skripsiNilais[$index]) ? old('nilai[' . $index . ']', $skripsiNilais[$index]->nilai) : '' }}"
                                    ...
                                    <td id="nilaiSkripsiTerbobot[{{ $index }}]" class="text-center">
                                        <?php
                                        $nilaiSkripsi = isset($skripsiNilais[$index]) ? $skripsiNilais[$index]->nilai : 0;
                                        $persentase = $skripsiParameter->persentase;
                                        $total = $nilaiSkripsi * ($persentase / 100);
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td id="totalNilaiSkripsiTerbobot" class="text-center font-bold">
                                    <?php
                                        $totalNilaiTerbobotSkripsi = 0;
                                        foreach ($skripsiParameters as $index => $skripsiParameter) {
                                            $persentase = $skripsiParameter->persentase;
                                            $nilaiSkripsi = $skripsiNilais[$index]->nilai ?? 0; // Ambil nilai jika sudah ada, jika tidak, gunakan nilai default 0
                                            $nilaiTerbobot = ($persentase / 100) * $nilaiSkripsi; // Hitung nilai terbobot
                                            $totalNilaiTerbobotSkripsi += $nilaiTerbobot;
                                        }
                                        echo number_format($totalNilaiTerbobotSkripsi, 2);
                                    ?>
                                </td>
                            </tr>
                        </tfoot>
                            ...
                            @foreach ($artikelParameters as $index => $artikelParameter)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <input required type="number" min="0" max="100"
                                            name="nilaiArtikel[{{ $index }}]"
                                            id="nilaiArtikel[{{ $index }}]"
                                            value="{{ isset($artikelNilais[$index]) ? old('nilai[' . $index . ']', $artikelNilais[$index]->nilai) : '' }}"
                                            class="input-field bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 "
                                    ...
                                    <td id="nilaiArtikelTerbobot[{{ $index }}]" class="text-center">
                                        <?php
                                        $nilaiArtikel = isset($artikelNilais[$index]) ? $artikelNilais[$index]->nilai : 0;
                                        $persentase = $artikelParameter->persentase;
                                        $total = $nilaiArtikel * ($persentase / 100);
                                        echo number_format($total,2);
                                        ?>
                                    </td>
                                </tr>
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td id="totalNilaiArtikelTerbobot" class="text-center font-bold">
                                    <?php
                                        $totalNilaiTerbobotArtikel = 0;
                                        foreach ($artikelParameters as $index => $artikelParameter) {
                                            $persentase = $artikelParameter->persentase;
                                            $nilaiArtikel = $artikelNilais[$index]->nilai ?? 0; // Ambil nilai jika sudah ada, jika tidak, gunakan nilai default 0
                                            $nilaiTerbobot = ($persentase / 100) * $nilaiArtikel; // Hitung nilai terbobot
                                            $totalNilaiTerbobotArtikel += $nilaiTerbobot;
                                        }
                                        echo number_format($totalNilaiTerbobotArtikel, 2);
                                    ?>
                                </td>
                            </tr>
                        </tfoot>
                            ...
                            @foreach ($presentasiParameters as $index => $presentasiParameter)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <input required type="number" min="0" max="100"
                                            name="nilaiPresentasi[{{ $index }}]"
                                            id="nilaiPresentasi[{{ $index }}]"
                                            value="{{ isset($presentasiNilais[$index]) ? old('nilai[' . $index . ']', $presentasiNilais[$index]->nilai) : '' }}"
                                            class="input-field bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 "
                                    ...
                                    <td id="nilaiPresentasiTerbobot[{{ $index }}]" class="text-center">
                                        <?php
                                        $nilaiPresentasi = isset($presentasiNilais[$index]) ? $presentasiNilais[$index]->nilai : 0;
                                        $persentase = $presentasiParameter->persentase;
                                        $total = $nilaiPresentasi * ($persentase / 100);
                                        echo number_format($total,2);
                                        ?>
                                    </td>
                                </tr>
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td id="totalNilaiPresentasiTerbobot" class="text-center font-bold">
                                    <?php
                                        $totalNilaiTerbobotPresentasi = 0;
                                        foreach ($presentasiParameters as $index => $presentasiParameter) {
                                            $persentase = $presentasiParameter->persentase;
                                            $nilaiPresentasi = $presentasiNilais[$index]->nilai ?? 0; // Ambil nilai jika sudah ada, jika tidak, gunakan nilai default 0
                                            $nilaiTerbobot = ($persentase / 100) * $nilaiPresentasi; // Hitung nilai terbobot
                                            $totalNilaiTerbobotPresentasi += $nilaiTerbobot;
                                        }
                                        echo number_format($totalNilaiTerbobotPresentasi, 2);
                                    ?>
                                </td>
                            </tr>
                        </tfoot>
                ...
                <div class="mb-5">
                    <p class="font-bold text-xl">
                        ...
                        <span id="totalNilaiKeseluruhan">
                            @php
                                $totalNilaiKeseluruhan = ($totalNilaiTerbobotSkripsi + $totalNilaiTerbobotArtikel + $totalNilaiTerbobotPresentasi) / 3;
                                echo number_format($totalNilaiKeseluruhan, 2);
                            @endphp
                        </span>
                    </p>
                    <label for="totalNilaiKeseluruhan" class="hidden">nilai</label>
                    <input type="hidden" name="totalNilaiKeseluruhan" id="inputTotalNilaiKeseluruhan" value="{{ $totalNilaiKeseluruhan }}">
                </div>
                <div class="mb-5">
                ...
                <div class="grid grid-cols-2 gap-10">
                    <a href="/dosen/penilaian-sidang/riwayat" type="button"
    ...
    @section('script')
        ...
        <script>
            function updateTotalNilaiKeseluruhan() {
                var totalNilaiSkripsiTerbobot = parseFloat(document.getElementById('totalNilaiSkripsiTerbobot').textContent) ||
                    0;
                var totalNilaiArtikelTerbobot = parseFloat(document.getElementById('totalNilaiArtikelTerbobot').textContent) ||
                    0;
                var totalNilaiPresentasiTerbobot = parseFloat(document.getElementById('totalNilaiPresentasiTerbobot')
                    .textContent) || 0;

                var totalNilaiKeseluruhan = (totalNilaiSkripsiTerbobot + totalNilaiArtikelTerbobot +
                    totalNilaiPresentasiTerbobot) / 3;
            ...
            function calculateSkripsiResult(index) {
                ...
                if (!isNaN(hasilSkripsi)) {
                    // Jika hasi$totalNilaiTerbobotSkripsilnya bukan NaN, tampilkan hasil pada elemen dengan id nilaiTerbobot[index]
                    ...
                    for (var i = 0; i < jumlahParameterSkripsi; i++) {
                        var nilaiSkripsiTerbobot = parseFloat(document.getElementById('nilaiSkripsiTerbobot[' + i + ']')
                            .textContent);
            ...
            function calculateArtikelResult(index) {
                ...
                if (!isNaN(hasilArtikel)) {
                    ...
                    for (var i = 0; i < jumlahParameterArtikel; i++) {
                        var nilaiArtikelTerbobot = parseFloat(document.getElementById('nilaiArtikelTerbobot[' + i + ']')
                            .textContent);
            ...
            function calculatePresentasiResult(index) {
                var persentasePresentasi = parseFloat(document.getElementById('persentasePresentasi[' + index + ']')
                    .textContent);
                ...
                if (!isNaN(hasilPresentasi)) {
                    ...
                    for (var i = 0; i < jumlahParameterPresentasi; i++) {
                        var nilaiPresentasiTerbobot = parseFloat(document.getElementById('nilaiPresentasiTerbobot[' + i + ']')
                            .textContent);
                        totalNilaiPresentasiTerbobot += isNaN(nilaiPresentasiTerbobot) ? 0 :
                            nilaiPresentasiTerbobot;
                    }
                    document.getElementById('totalNilaiPresentasiTerbobot').textContent = totalNilaiPresentasiTerbobot.toFixed(
                        2);
                } else {
