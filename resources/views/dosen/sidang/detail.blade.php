<x-app-layout>
    @section('title', 'Detail Sidang Akhir')
    <x-main-card>
        <div class="overflow-x-auto miniscrollbar">
            <table class="table table-striped table-hover" style="width: 100%">
                <caption class="caption-top font-bold text-xl text-slate-900 uppercase mb-2 tracking-wide">
                    Detail Sidang Akhir
                    <hr class="mt-2 border border-l border-slate-900">
                </caption>
                <tr>
                    <th style="width: 28%">Nama</th>
                    <td class="font-bold" style="width: 1%">:</td>
                    <td>{{ $sidang->mahasiswa->nama_lengkap }}</td>
                </tr>
                <tr>
                    <th>NIM</th>
                    <td class="font-bold">:</td>
                    <td>{{ $sidang->mahasiswa->nim }}</td>
                </tr>
                <tr>
                    <th>Judul</th>
                    <td class="font-bold">:</td>
                    <td class="whitespace-pre-wrap">{{ $sidang->tugas_akhir->judul }}</td>
                </tr>
                <tr>
                    <th>Studi Kasus</th>
                    <td class="font-bold">:</td>
                    <td>{{ $sidang->tugas_akhir->studi_kasus }}</td>
                </tr>
                <tr>
                    <th>Metode</th>
                    <td class="font-bold">:</td>
                    <td>{{ $sidang->tugas_akhir->metode }}</td>
                </tr>
                @if ($sidang->status_kelulusan == 'LULUS' || $sidang->status_kelulusan == 'TIDAK LULUS')
                    <tr>
                        <th style="width: 28%" class="font-medium">Berita Acara Sidang Sarjana</th>
                        <td class="font-medium" style="width: 1%">:</td>
                        <td>
                            <a href="{{ route('dosen.cetak', $sidang->slug) }}" target="_blank"
                                class="text-blue-600 underline">
                                Berita Acara Sidang Sarjana.pdf
                            </a>
                        </td>
                    </tr>
                    <tr>
                @endif
                <tr>
                    <th>Dosen Pembimbing Akademik</th>
                    <td class="font-bold">:</td>
                    <td>{{ $sidang->tugas_akhir->dosen_PA->nama_dosen }}</td>
                </tr>
                <tr>
                    <th>Dosen Pembimbing 1</th>
                    <td class="font-bold">:</td>
                    <td>{{ $sidang->tugas_akhir->dosen_pembimbing_1->nama_dosen }}</td>
                </tr>
                <tr>
                    <th>Dosen Pembimbing 2</th>
                    <td class="font-bold">:</td>
                    <td>{{ $sidang->tugas_akhir->dosen_pembimbing_2->nama_dosen }}</td>
                </tr>
                <tr>
                    <th>Dosen Penguji 1</th>
                    <td class="font-bold">:</td>
                    <td>
                        @if ($sidang->tugas_akhir->dosen_penguji_1_id === null)
                            -
                        @else
                            {{ $sidang->tugas_akhir->dosen_penguji_1->nama_dosen }}
                        @endif
                    </td>
                </tr>
                <tr>
                    <th>Dosen Penguji 2</th>
                    <td class="font-bold">:</td>
                    <td>
                        @if ($sidang->tugas_akhir->dosen_penguji_2_id === null)
                            -
                        @else
                            {{ $sidang->tugas_akhir->dosen_penguji_2->nama_dosen }}
                        @endif
                    </td>
                </tr>
                <tr>
                    <th>Tempat Pelaksanaan</th>
                    <td class="font-bold">:</td>
                    <td>
                        @if ($sidang->tempat_pelaksanaan === null)
                            -
                        @else
                            {{ $sidang->tempat_pelaksanaan }}
                        @endif
                    </td>
                </tr>
                <tr>
                    <th>Tanggal Pelaksanaan</th>
                    <td class="font-bold">:</td>
                    <td>
                        @if ($sidang->tanggal_pelaksanaan === null)
                            -
                        @else
                            {{ $sidang->tanggal_pelaksanaan }}
                        @endif
                    </td>
                </tr>
                <tr>
                    <th>Jam Pelaksanaan</th>
                    <td class="font-bold">:</td>
                    <td>
                        @if ($sidang->jam_pelaksanaan === null)
                            -
                        @else
                            {{ $sidang->jam_pelaksanaan }}
                        @endif
                    </td>
                </tr>
                <tr>
                    <th>Dokumen Sidang Akhir</th>
                    <td class="font-bold">:</td>
                    <td>@php $dokumenSidang = $sidang ? $sidang->dokumen_ta : null; @endphp
                        <x-new-modal :file="$dokumenSidang" />
                    </td>
                </tr>
                <tr>
                    <th>Dokumen Artikel Jurnal</th>
                    <td class="font-bold">:</td>
                    <td>
                        @php
                            $lampiranSidang = $sidang ? json_decode($sidang->file_lampiran) : null;
                            $artikel = $lampiranSidang[1] ?? null;
                        @endphp
                        <x-new-modal :file="$artikel" />
                    </td>
                </tr>
            </table>
        </div>
        @if ($sidang->tugas_akhir->dosen_pembimbing_1_id == $dosenId && $sidang->total_nilai_pembimbing_1 !== null)
            <a href="/dosen/penilaian-sidang/riwayat" type="button"
                class="float-end mt-3 text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Kembali
            </a>
        @elseif ($sidang->tugas_akhir->dosen_pembimbing_2_id == $dosenId && $sidang->total_nilai_pembimbing_2 !== null)
            <a href="/dosen/penilaian-sidang/riwayat" type="button"
                class="float-end mt-3 text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Kembali
            </a>
        @elseif ($sidang->tugas_akhir->dosen_penguji_1_id == $dosenId && $sidang->total_nilai_penguji_1 !== null)
            <a href="/dosen/penilaian-sidang/riwayat" type="button"
                class="float-end mt-3 text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Kembali
            </a>
        @elseif ($sidang->tugas_akhir->dosen_penguji_2_id == $dosenId && $sidang->total_nilai_penguji_2 !== null)
            <a href="/dosen/penilaian-sidang/riwayat" type="button"
                class="float-end mt-3 text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Kembali
            </a>
        @else
            <a href="/dosen/penilaian-sidang" type="button"
                class="float-end mt-3 text-white bg-gradient-to-r from-cyan-500 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Kembali
            </a>
        @endif
        @if ($sidang->tugas_akhir->dosen_pembimbing_1_id == $dosenId && $sidang->total_nilai_pembimbing_1 == null)
            <a href="/dosen/penilaian-sidang/nilai/{{ $sidang->slug }}" type="button"
                class="float-end mt-3 text-white bg-gradient-to-r from-orange-500 to-orange-700 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-orange-300 dark:focus:ring-orange-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Nilai
            </a>
        @elseif ($sidang->tugas_akhir->dosen_pembimbing_2_id == $dosenId && $sidang->total_nilai_pembimbing_2 == null)
            <a href="/dosen/penilaian-sidang/nilai/{{ $sidang->slug }}" type="button"
                class="float-end mt-3 text-white bg-gradient-to-r from-orange-500 to-orange-700 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-orange-300 dark:focus:ring-orange-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Nilai
            </a>
        @elseif ($sidang->tugas_akhir->dosen_penguji_1_id == $dosenId && $sidang->total_nilai_penguji_1 == null)
            <a href="/dosen/penilaian-sidang/nilai/{{ $sidang->slug }}" type="button"
                class="float-end mt-3 text-white bg-gradient-to-r from-orange-500 to-orange-700 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-orange-300 dark:focus:ring-orange-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Nilai
            </a>
        @elseif ($sidang->tugas_akhir->dosen_penguji_2_id == $dosenId && $sidang->total_nilai_penguji_2 == null)
            <a href="/dosen/penilaian-sidang/nilai/{{ $sidang->slug }}" type="button"
                class="float-end mt-3 text-white bg-gradient-to-r from-orange-500 to-orange-700 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-orange-300 dark:focus:ring-orange-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Nilai
            </a>
            {{-- @elseif (
            ($sidang->tugas_akhir->dosen_penguji_1_id == $dosenId || $sidang->tugas_akhir->dosen_penguji_2_id == $dosenId) &&
                $sidang->total_nilai_penguji_1 != null) --}}
        @endif
        @if (
            $sidang->is_publish == false &&
                ($sidang->tugas_akhir->dosen_pembimbing_1_id == $dosenId ||
                    $sidang->tugas_akhir->dosen_pembimbing__id == $dosenId) &&
                ($sidang->total_nilai_penguji_1 != null &&
                    $sidang->total_nilai_penguji_2 != null &&
                    $sidang->total_nilai_pembimbing_1 != null &&
                    $sidang->total_nilai_pembimbing_2 != null))
            <a href="/dosen/penilaian-sidang/publish/{{ $sidang->slug }}" type="button"
                class="float-end mt-3 text-white bg-gradient-to-r from-blue-500 to-blue-700 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-orange-300 dark:focus:ring-orange-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Publish
            </a>
        @elseif ($sidang->is_publish == true)
            <a href="/dosen/penilaian-sidang/unpublish/{{ $sidang->slug }}" type="button"
                class="float-end mt-3 text-white bg-gradient-to-r from-blue-500 to-blue-700 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-orange-300 dark:focus:ring-orange-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2">
                Unpublish
            </a>
        @endif


    </x-main-card>
    @section('script')
        <script>
            dropDownSidang()

            function show(file) {
                var modal = document.getElementById('lampiran_' + file);
                modal.showModal();
            }

            function closeLampiran(file) {
                var modal = document.getElementById('lampiran_' + file);
                modal.close();
            }
        </script>
    @endsection
</x-app-layout>
