<x-app-layout>
    @section('title', 'Berita Acara Sidang Akhir')
    @section('head')
                ...
                <table id="myTable" class="table table-striped table-hover table-bordered">
                    ...
                    <tbody>
                        @foreach ($seminarSidangs as $sidang)
                            <tr>
                                <td class="">{{ $sidang->tanggal_pelaksanaan }}</td>
                                <td class="">{{ $sidang->mahasiswa->nama_lengkap }}</td>
                                <td class="">{{ $sidang->mahasiswa->nim }}</td>
                                <td class="">{{ $sidang->tugas_akhir->judul }}</td>
                                <td class="text-center">
                                    <div
                                        ...
                                        <span class="font-medium">{{ $sidang->status_kelulusan }}</span>
                                    </div>
                                </td>
                                <td class="whitespace-nowrap text-center">
                                    @if ($sidang->no_surat)
                                        <a href="/admin/sidang/berita-acara/{{ $sidang->slug }}"
                                            data-tooltip-target="detail_{{ $sidang->slug }}"
                                            data-tooltip-placement="bottom" type="button" target="_blank"
                                    ...
                                    @else
                                        <a href="{{ route('admin.sidang.detail', ['slug' => $sidang->slug]) }}"
                                            data-tooltip-target="noba_{{ $sidang->slug }}"
                                            data-tooltip-placement="bottom" type="button" target="_blank"
                                            class="cursor-pointer text-white bg-red-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium rounded-lg text-sm w-8 h-7 inline-flex items-center justify-center">
                                            <i class="bi bi-pencil"></i>
                                        </a>

                                        <div id="noba_{{ $sidang->slug }}" role="tooltip"
                                            class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                            Isi No Surat Dahulu
                                            <div class="tooltip-arrow" data-popper-arrow></div>
                                        </div>
